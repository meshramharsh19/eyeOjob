...
// Cohere's OpenAI-compatibility endpoint, not their native /v2/chat shape —
// same reasoning as ai.extractor.js's server-chain caller: reuses the same
// request/response handling as every other OpenAI-compatible provider here.
class CohereAdapter extends BaseAIProvider {
  constructor() {
    super(providersConfig.cohere);
  }
  ...
  async _call(apiKey, model, prompt) {
    ...
    if (!res.ok) {
      const err = new Error(`Cohere request failed: ${res.status}`);
...
module.exports = new CohereAdapter();
